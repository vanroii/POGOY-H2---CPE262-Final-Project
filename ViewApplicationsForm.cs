...
namespace POGOY_H2___CPE262_Final_Project
{
    public partial class ViewApplicationsForm : Form
    {
        ...
        private void LoadApplications()
        {
            ...
            {
                con.Open();
                string query = @"SELECT ApplicationID, JobID, UserID, Status, DateApplied FROM Applications";
        ...
        private void btnRemoveApplication_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show("Remove this application?","Confirm",MessageBoxButtons.YesNo);
            if (result == DialogResult.No) return;
            int appId = Convert.ToInt32(dgvApplications.CurrentRow.Cells["ApplicationID"].Value);
            using (OleDbConnection con = DBConnection.GetConnection())
            {
                con.Open();
